...
| 离线运行测试 | 28 通过、0 失败；普通测试中的 PostgreSQL 项跳过，另行真实执行。见 [runtime.tap](../pi/runtime.tap) |
| PostgreSQL 独立集成测试 | 临时真实数据库中 1 通过、0 失败。见 [postgres.tap](../pi/postgres.tap) |
...
| 生产依赖审计 | 0 已报告漏洞。见 [dependency-audit.json](../pi/dependency-audit.json) |
...
启动、模型配置、数据库初始化与人工程序登记见 [README.md](../../../docs/old/implementation-notes/README.md)。建议先在默认离线模式复核消息→任务→授权→执行→反馈→重启恢复，再使用独立角色密钥进行真实模型复核。
